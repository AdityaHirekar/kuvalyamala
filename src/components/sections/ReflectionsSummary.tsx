...
export default function ReflectionsSummary() {
    ...
    const clearReflections = () => {
        if (window.confirm(t.reflections.resetConfirm)) {
        ...
        <Section id="reflections-summary" background="stone" className="py-24 border-t border-maroon/10">
            <div className="max-w-4xl mx-auto">
                <div className="text-center mb-16">
                    ...
                    <h2 className="text-3xl md:text-5xl font-serif font-bold text-ink mb-6">{t.reflections.title}</h2>
                                ...
                                <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-3 mb-3">
                                            <span className="text-xs font-bold uppercase tracking-widest text-maroon">{t.ui.chapter} {chapter.id}</span>
                                            <div className="h-px w-8 bg-maroon/20" />
                                            <h4 className="font-serif font-bold text-ink/80">{chapter.title}</h4>
                                        </div>

                                        <div className="mb-4">
                                            <p className="text-sm text-ink/50 italic mb-2">{t.reflections.question}</p>
                                            <p className="font-serif text-ink">{chapter.reflection.question}</p>
                                        </div>

                                        <div className="bg-maroon/5 p-4 rounded-md inline-block">
                                            <p className="text-sm text-ink/50 italic mb-1">{t.reflections.yourChoice}</p>
                                    ...
                                    {reflection.note && (
                                        <div className="flex-1 md:border-l border-ink/10 md:pl-8">
                                            <p className="text-sm text-ink/50 italic mb-3">{t.reflections.yourNote}</p>
